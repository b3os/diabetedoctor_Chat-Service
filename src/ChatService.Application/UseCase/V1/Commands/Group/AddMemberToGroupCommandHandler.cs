...
public class AddMemberToGroupCommandHandler(IUnitOfWork unitOfWork, IGroupRepository groupRepository, IUserRepository userRepository) : ICommandHandler<AddMemberToGroupCommand>
{
    public async Task<Result> Handle(AddMemberToGroupCommand request, CancellationToken cancellationToken)
    {
        ...
        var group = await groupRepository.FindSingleAsync(group => group.Id == ObjectId.Parse(request.GroupId)
        ...
        return Result.Success(new Response( GroupMessage.AddMemberToGroupSuccessfully.GetMessage().Code, GroupMessage.AddMemberToGroupSuccessfully.GetMessage().Message));
    }
}