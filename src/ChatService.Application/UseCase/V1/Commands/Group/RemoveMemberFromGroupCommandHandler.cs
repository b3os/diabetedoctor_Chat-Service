    ...
    : ICommandHandler<RemoveMemberFromGroupCommand>
{
    public async Task<Result> Handle(RemoveMemberFromGroupCommand request, CancellationToken cancellationToken)
    {
        ...
        await EnsureAccessGroupPermissionAsync(request.GroupId, request.AdminId!, cancellationToken);
    ...
    private async Task EnsureAccessGroupPermissionAsync(ObjectId groupId, string executorId, CancellationToken cancellationToken)
    {
    ...
    private GroupMemberRemovedEvent MapToEvent(RemoveMemberFromGroupCommand command)
    {
